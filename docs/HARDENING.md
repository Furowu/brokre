...
- Shrink the window where decrypted passwords exist in the **long-lived** `brokr` parent (Unix: parent never decrypts for saved records; a short `brokr --internal-injector` child decrypts and writes once to the PTY).
...
| Variable | Effect |
|----------|--------|
| `BROKR_DISABLE_HARDENING=1` | Skips OS hardening in the main process and **refuses vault injector** (injector exits; no password injection). |
| `BROKR_SOFT_MEMLOCK=1` (Linux only) | If `mlockall` fails under `Enforce`, allow startup with a warning instead of exiting. |
| `BROKR_INJECTOR_EXE` (Unix, advanced) | Path to the `brokr` binary used to spawn `--internal-injector`. Defaults to `std::env::current_exe()`. Integration tests set this when the PTY runs inside a non-`brokr` test harness so the real CLI binary is executed. |

## Build profiles

- **Debug** (`cargo build` / `cargo test`): `HardeningMode::WarnOnly` — hardening failures are warnings where possible (e.g. `mlockall`).
- **Release** (`cargo build --release`): `HardeningMode::Enforce` — Linux `TracerPid != 0` or `mlockall` failure (without `BROKR_SOFT_MEMLOCK`) aborts startup.
...
1. **Linux release + memlock:** `cargo build --release && ./target/release/brokr list` — expect failure if `mlockall` cannot lock and `BROKR_SOFT_MEMLOCK` is unset; success with `BROKR_SOFT_MEMLOCK=1`.
2. **Linux core:** After a normal `brokr ssh` session, `gcore $(pidof brokr)` should fail or produce a core without vault password strings (parent never decrypts on Unix).
3. **macOS debugger:** Release `brokr` should resist casual `lldb` attach after `PT_DENY_ATTACH` (SIP / permissions may still allow root).
4. **Container:** If `mlockall` fails, set `BROKR_SOFT_MEMLOCK=1` or raise `memlock` ulimit / `IPC_LOCK` capability.
