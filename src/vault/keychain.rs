...
const SERVICE: &str = "brokr";
const MASTER_KEK_ACCOUNT: &str = "master_kek";
const AUDIT_HMAC_ACCOUNT: &str = "audit_hmac";

fn fallback_path(account: &str) -> PathBuf {
    crate::utils::paths::brokr_home().join(format!(".{}", account))
}

fn decode_key(b64: &str) -> Result<[u8; 32]> {
    let bytes = base64ct::Base64::decode_vec(b64).map_err(|e| BrokrError::Crypto(e.to_string()))?;
    if bytes.len() != 32 {
        return Err(BrokrError::Crypto("corrupt key length".into()));
    }
    let mut key = [0u8; 32];
...
fn allow_file_fallback() -> bool {
    ...
    if std::env::var("BROKR_ALLOW_FILE_KEYCHAIN").is_ok() {
        return true;
    }
    // Explicit opt-in to keychain (overrides platform default).
    if std::env::var("BROKR_USE_KEYCHAIN").is_ok() {
        return false;
    }
...
fn get_or_init(account: &str) -> Result<[u8; 32]> {
    ...
    let entry = Entry::new(SERVICE, account).map_err(|e| BrokrError::Vault(e.to_string()))?;
...
pub fn rotate_master_kek() -> Result<()> {
    Err(BrokrError::Vault("rotate not implemented".into()))
}
