...
mod imp {
        ...
        pub fn sweep_idle(&mut self) {
            ...
            self.sessions.retain(|key, entry| {
                ...
                if idle || expired || dead {
                    audit_action("mcp/elevated-session/expire", key, None, None, 0);
                    entry.pty.kill();
                    false
                } else {
                    true
        ...
        pub fn run(
            ...
            if cmd.is_empty() {
                return Err(BrokreError::Runtime(
                    "elevated session: command is required (use session=close to end session)"
                        .into(),
                ));
            }
    ...
    fn idle_expires_iso(idle: Duration) -> String {
        (Utc::now()
            + ChronoDuration::from_std(idle).unwrap_or_else(|_| ChronoDuration::minutes(10)))
        .to_rfc3339()
    }
    ...
    fn open_session(
        ...
        let ssh = which::which("ssh")
            .map_err(|_| BrokreError::Runtime("ssh: command not found".into()))?;
