...
/// Run the injector entrypoint (`brokr --internal-injector …`). Exits the process.
...
fn run_internal_injector() -> Result<()> {
    if hardening::hardening_disabled_by_env() {
        return Err(BrokrError::Runtime(
            "injector: BROKR_DISABLE_HARDENING=1 — refusing to decrypt".into(),
        ));
    }
    ...
    let id_str = args
        .next()
        .ok_or_else(|| BrokrError::Runtime("injector: missing record id".into()))?;
    let ppid_str = args
        .next()
        .ok_or_else(|| BrokrError::Runtime("injector: missing parent pid".into()))?;
    let field = args.next().unwrap_or_else(|| "password".into());

    let id: Uuid = id_str
        .parse()
        .map_err(|_| BrokrError::Runtime("injector: bad record id".into()))?;
    let expected_ppid: u32 = ppid_str
        .parse()
        .map_err(|_| BrokrError::Runtime("injector: bad parent pid".into()))?;

    let actual_ppid = unsafe { libc::getppid() } as u32;
    if actual_ppid != expected_ppid {
        return Err(BrokrError::Runtime("injector: parent pid mismatch".into()));
    }

    let fd_pty = 3i32;
    let fd_tok = 4i32;
    if unsafe { libc::isatty(fd_pty) } != 1 {
        return Err(BrokrError::Runtime("injector: fd 3 is not a tty".into()));
    }

    let mut tok = vec![0u8; INJECTOR_TOKEN_MAX];
    let n = unsafe { libc::read(fd_tok, tok.as_mut_ptr().cast(), tok.len().saturating_sub(1)) };
    if n <= 0 {
        return Err(BrokrError::Runtime("injector: empty token".into()));
    }
    ...
    let rec = store
        .get_by_id(&id)?
        .ok_or_else(|| BrokrError::Vault("injector: record not found".into()))?;

    let master = get_or_init_master_kek()?;
    let fields = decrypt_for_exec(&rec.crypto, &master)?;
    let pw = fields
        .get(&field)
        .ok_or_else(|| BrokrError::Vault(format!("injector: no {} field", field)))?;
