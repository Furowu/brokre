...
pub enum IdleBehavior {
    /// Standalone `brokr manage` — exit the whole process.
    ExitProcess,
    /// Embedded in `brokr mcp` — invalidate session; MCP keeps running.
    LogOnly,
}
...
pub fn run_manage_server_with(options: ManageServerOptions) -> Result<ManageServer> {
    ...
    let port = listener.local_addr().map_err(BrokrError::Io)?.port();
    let server = Server::from_listener(listener, None)
        .map_err(|e| BrokrError::Runtime(e.to_string()))?;
    ...
    if log_full_url {
        ...
        eprintln!(
            "brokr manage: press Ctrl+C to stop (idle timeout {} min)",
            IDLE_LIMIT_SECS / 60
        );
    } else {
        eprintln!(
            "brokr manage: http://127.0.0.1:{}/ (session token omitted from logs — use browser)",
            port
        );
        eprintln!(
            "brokr manage: idle timeout {} min (embedded — session expires, MCP keeps running)",
            IDLE_LIMIT_SECS / 60
        );
    }
            ...
            if state_idle.idle_secs() >= IDLE_LIMIT_SECS {
                match idle_behavior {
                    ...
                    IdleBehavior::LogOnly => {
                        ...
                        eprintln!(
                            "brokr manage: idle timeout — session expired (embedded server no longer accepts auth)"
                        );
                        break;
                    }
                }
...
fn bind_localhost_listener() -> Result<TcpListener> {
    ...
    Err(BrokrError::Runtime("no free localhost port".into()))
}

fn try_bind(port: u16) -> Result<TcpListener> {
    let addr = format!("127.0.0.1:{}", port);
    TcpListener::bind(&addr).map_err(BrokrError::Io)
}
