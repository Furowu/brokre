...
| **支持的 IDE** | Cursor、VS Code、VS Code Insiders、Claude Code、Claude Desktop、Trae、Kimi Code、Windsurf、OpenClaw、Codex CLI、Gemini CLI、Hermes Agent、Continue、Zed — 详见 [packages/brokre-mcp/README.md](packages/brokre-mcp/README.md)。 |
...
**Windows：** `npm install -g brokre`，开新终端后执行 `brokre --version`（首次运行如缺原生 CLI 会自动下载）。本地 `npm i brokre`（无 `-g`）不会进 PATH。
...
| Continue | `~/.continue/config.yaml` 或 `~/.continue/mcpServers/brokre.yaml` |
| Zed | `~/.config/zed/settings.json`（`context_servers`） |
| ChatGPT Desktop | _（不支持 — 仅远程 Connectors UI）_ |
| Grok Bot | _（提示 — connectors/plugins；无 mcp.json）_ |
...
ChatGPT Desktop 无本地 stdio 配置（仅远程 Connectors）。Grok Bot 使用 connectors/plugins — 可让 Bot 添加 `npx -y brokre@latest`。
